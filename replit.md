...
- **Theme System**: 6 themes selectable from Account settings: Metallic Glass (default), Frosted Aurora, Brushed Steel, Smoky Glass, Noir Minimal, Liquid Dark. ThemeContext (src/contexts/ThemeContext.tsx) with localStorage persistence (`x247_theme` key), applies `data-theme` attribute on `<html>`. CSS overrides per theme in index.css using `[data-theme="..."]` selectors.
