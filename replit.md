...
- **Profile System**: UserProfileCard component (src/components/UserProfileCard.tsx) — card-style profile with avatar/initials, name, verified badge, bio, stats, share button. Users can set bio, avatar (file upload), profile slug, public/private visibility. Public profiles accessible at /profile/:slug. Shareable via clipboard/native share. Badge system with 8 earnable badges (Early Adopter, Streak Master, First Win, etc.) stored in user_badges table. Membership purchase (Basic/Premium) grants verified badge.
