...
- **Pages**: Home, Offers, Partners, Admin Login, Control Panel, Contest Hub, Giveaway Entry Form, Winners Hall of Fame, User Account (Login/Register, Dashboard, Profile Editor, Badges, Membership, Giveaway History), Public User Profile, Community, Referral Partner Program, Referral Dashboard, Pricing (`/pricing`), How It Works (`/how-it-works` — premium multi-section page: hero, 3-step process, entry mechanics, draw transparency, tier ladder, partner ecosystem, FAQ, CTA).
- **User Profile System**: UserProfileCard component with avatar/initials, name, verified badge, bio, stats, and share button. Public profiles at `/profile/:slug`. Eight earnable badges. Public profile page has: (1) full-width identity header band (glass-card) with avatar ring+glow, name, tier pill, meta chips, Follow/Share buttons, stats strip; (2) two-column grid with TradingCard sidebar (no corner arcs — crosshair reticle + initials glow ring) + right column cards (Featured Badge v2, Stats 4-grid, Badges Wall flip cards, Activity Heatmap, URL card, CTA); Level & XP and Bio cards in sidebar below TradingCard.
- **Account Dashboard**: Full-page, edge-to-edge layout (no box/container). Sticky topbar (64px, height-fixed, blurred) at top:70px. Sticky sidebar (248px wide, lg+ screens) with mini user profile card, section labels, nav items, and Sign Out button at bottom. Mobile tab row below topbar. Content area max-width 960px. FollowListModal and win-popup use `position:fixed` safely (no overflow:hidden on dash-frame). Tabs: Overview, Profile, Subscription, Entries, Referrals, Billing, Settings. All 9 themes fully overridden.
