                  ...
                  <div className="relative z-[2] flex flex-col md:flex-row md:items-center gap-6 md:gap-10">
                    ...
                    <div className="flex-1">
                      <div className="flex items-center gap-3 mb-3">
                        <span className="text-[10px] sm:text-xs font-display font-medium text-foreground/30 uppercase tracking-widest">{offer.tag}</span>
                        <span className="w-1.5 h-1.5 rounded-full bg-white/40 animate-pulse"></span>
                        <span className="text-[10px] font-display text-foreground/45 uppercase tracking-wider">Live</span>
                        <div className="premium-badge premium-badge-hot ml-auto sm:ml-4">
                          <Crown className="w-3 h-3 mr-1.5" />
                          Featured
                        </div>
                      </div>
                      <h3 className="text-2xl sm:text-3xl md:text-4xl font-display font-light text-foreground mb-3">{offer.title}</h3>
                      <p className="text-foreground/40 font-light mb-6 text-sm sm:text-base leading-relaxed max-w-2xl">{offer.desc}</p>
                      <div className="flex flex-wrap gap-3">
                        {offer.details.map((d, di) => (
                          <div key={di} className="stat-card !p-2.5 !px-4 !rounded-xl flex items-center gap-2">
                            <CheckCircle2 className="w-3.5 h-3.5 text-foreground/25 shrink-0" />
                            <span className="text-xs text-foreground/55">{d}</span>
                          </div>
                        ))}
                      </div>
                    </div>
                    <div className="hidden md:flex flex-col items-center">
                      <span className="text-6xl font-display font-light text-foreground/[0.04] leading-none">{offer.number}</span>
                    </div>
                  </div>
                </div>
              );
                  ...
                  <div className={`glass-card ${offer.highlight ? 'glass-card-featured' : ''} ${classes.card} p-5 sm:p-8 group h-full relative`}>
                    ...
                    <div className="relative z-[2] flex flex-col h-full">
                      <div className="flex items-start justify-between mb-5">
                        ...
                        <span className="text-4xl sm:text-5xl font-display font-light text-foreground/[0.03] leading-none mt-[-4px]">{offer.number}</span>
                      </div>
                      <div className="flex-1">
                        <div className="flex items-center gap-2 mb-3">
                          <span className="text-[10px] sm:text-xs font-display font-medium text-foreground/25 uppercase tracking-widest">{offer.tag}</span>
                          <span className="w-1.5 h-1.5 rounded-full bg-white/30 animate-pulse"></span>
                          <span className="text-[10px] font-display text-foreground/40 uppercase tracking-wider">Live</span>
                        </div>
                        <h3 className="text-xl sm:text-2xl font-display font-light text-foreground mb-3">{offer.title}</h3>
                        <p className="text-foreground/35 font-light mb-6 text-xs sm:text-sm leading-relaxed">{offer.desc}</p>
                      </div>
                        ...
                        <ul className="space-y-2.5 text-xs text-foreground/50">
                          {offer.details.map((d, di) => (
                            <li key={di} className="flex items-center">
                              <CheckCircle2 className="w-3.5 h-3.5 mr-2.5 text-foreground/20 shrink-0" />
                              {d}
                            </li>
                          ))}
                        </ul>
              ...
              <div className="marquee-content">
                {["2x Entry Bonus", "Refer & Earn", "Early Bird Drops", "Partner Cashback", "Weekend Mega Draw", "Flash Giveaways", "Community Rewards", "Exclusive Merch"].map((text, i) => (
                  <div key={`m1-${i}`} className="flex items-center gap-8 whitespace-nowrap">
                    <span className="text-lg sm:text-2xl font-display font-light text-foreground/30 uppercase tracking-wider">{text}</span>
                    <span className="text-foreground/15 text-xl font-light">✦</span>
                  </div>
                ))}
              </div>
              <div className="marquee-content">
          ...
          >
            <div className="glass-card glass-card-accent-navy p-8 sm:p-14 max-w-2xl mx-auto relative">
              ...
              <div className="relative z-[2]">
                ...
                <h3 className="text-xl sm:text-3xl font-display font-light text-foreground mb-4">Ready to Enter?</h3>
                <p className="text-foreground/45 font-light mb-8 text-sm leading-relaxed max-w-md mx-auto">
