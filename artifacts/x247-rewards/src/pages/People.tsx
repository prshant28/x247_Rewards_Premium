    ...
    <div className="min-h-screen bg-black text-foreground">
      ...
      <main className="relative z-10 pt-[70px] pb-20 sm:pb-32">
        ...
        <div className="container mx-auto px-4 max-w-5xl">
            ...
            <div className="glass-card p-4 sm:p-5">
              ...
              <div className="relative z-[2] flex flex-col sm:flex-row items-center justify-between gap-4">
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-sm text-foreground/50 font-light">Network Overview</span>
                </div>
                <div className="flex items-center gap-6 sm:gap-10">
                  <div className="text-center">
                    <div className="text-xl sm:text-2xl font-display font-light text-foreground">{discover.length || "—"}</div>
                    <div className="text-[10px] text-foreground/30 uppercase tracking-widest font-display">Suggested</div>
                  </div>
                  <div className="w-px h-8 bg-white/[0.06]" />
                  <div className="text-center">
                    <div className="text-xl sm:text-2xl font-display font-light text-foreground">{followers.length || "—"}</div>
                    <div className="text-[10px] text-foreground/30 uppercase tracking-widest font-display">Followers</div>
                  </div>
                  <div className="w-px h-8 bg-white/[0.06]" />
                  <div className="text-center">
                    <div className="text-xl sm:text-2xl font-display font-light text-foreground">{following.length || "—"}</div>
                    <div className="text-[10px] text-foreground/30 uppercase tracking-widest font-display">Following</div>
                  </div>
                </div>
              </div>
            </div>
          ...
          {/* ── Filter row (Partners ctx-filterbar DNA) + Refresh ── */}
          <motion.div initial="hidden" animate="visible" variants={fadeUp} className="mb-6 sm:mb-8 ctx-filterbar">
            <div className="flex items-center gap-2 flex-wrap">
              <div className="flex items-center gap-1.5 mr-1 text-foreground/45">
                <SlidersHorizontal className="w-3.5 h-3.5" />
                <span className="text-[10px] uppercase tracking-widest font-display">View</span>
              </div>
              {tabs.map((t) => (
                <button
                  key={t.id}
                  ...
                  className="ctx-status-chip px-4 py-1.5 rounded-full text-[11px] font-display font-light tracking-wide transition-all inline-flex items-center gap-1.5"
                >
                  <t.icon className="w-3 h-3" />
                  <span>{t.label}</span>
                  {t.count > 0 && (
                    <span className="text-foreground/45">· {formatCount(t.count)}</span>
                  )}
                </button>
              ))}
              <button
                ...
                className="ml-auto inline-flex items-center justify-center w-8 h-8 rounded-full bg-white/[0.04] border border-white/[0.08] text-foreground/55 hover:text-foreground/85 hover:bg-white/[0.07] transition-all disabled:opacity-40"
                aria-label="Refresh"
                title="Refresh"
              >
          ...
          {!query && filtered.length > 0 && (
            <div className="flex items-center gap-2 mb-5 text-foreground/45">
              {tab === "discover" && <Sparkles className="w-3.5 h-3.5" />}
              {tab === "followers" && <Users className="w-3.5 h-3.5" />}
              {tab === "following" && <UserCheck className="w-3.5 h-3.5" />}
              <span className="text-[10px] uppercase tracking-[0.18em] font-display">
                {tab === "discover" && "Suggested for You"}
                {tab === "followers" && "People following you"}
